function loc = get_atomloc(phase,include_equiv)
    ...
    loc = symopsHardCoded(phase,include_equiv);
    
end

function locGross = symopsHardCoded(phase, include_equiv)
    ...
    elseif ~include_equiv
        loclist('bcc') = [0,0,0]; % position a
        loclist('fcc') = [0,0,0]; % position a
        loclist('a15') = [0,0,0; % position a
                        0, 0.5, 0.25]; % position c
        loclist('c15') = [0.125,0.125,0.125; %pos a
                        0.5, 0.5, 0.5]; %pos d
        loclist('c14') = [];
        
        % TO BE CONTINUED! but only if we decide that we want to isolate
        % symmetry inequivalent particles
    end
